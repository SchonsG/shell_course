...
if [[ "${UID}" -ne '0' ]]
then
  echo 'For run this script, is necessary execute with root privileges' >&2
  exit 1
fi
...
if [[ "${#}" -eq 0 ]]
then
  echo "Usage: ${APPNAME} USER_NAME [COMMENT]..." >&2
  echo 'Create an account in your local system with the name of USER_NAME and a comments field of COMMENT.' >&2
  exit 1
fi
